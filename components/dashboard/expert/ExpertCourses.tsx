...
export default function ExpertCourses() {
        ...
        modules: [
          {
            ...
            quiz: {
              questions: [
                ...
                {
                  id: 2,
                  question: "Which metric is most important for measuring social media success?",
                  options: [
                    "Number of posts",
                    "Follower count",
                    "ROI",
  ...
  const handleQuestionChange = (moduleIndex: number, questionIndex: number, field: string, value: string | number) => {
    const updatedModules = [...courseForm.modules];
    if (field === 'options') {
      const optionIndex = parseInt(value.toString());
      updatedModules[moduleIndex].quiz.questions[questionIndex].correctAnswer = optionIndex;
    } else {
  ...
  const handleRemoveQuestion = (moduleIndex: number, questionIndex: number) => {
    const updatedModules = [...courseForm.modules];
    updatedModules[moduleIndex].quiz.questions = updatedModules[moduleIndex].quiz.questions.filter(
      (_, i) => i !== questionIndex
    );
    setCourseForm({
      ...courseForm,
                            ...
                            <CardContent className="space-y-4 pt-4">
                              <div className="flex justify-between items-start">
                                ...
                                <Button
                                  ...
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleRemoveQuestion(moduleIndex, questionIndex)}
                                >
                                  <Trash className="h-4 w-4" />
                                </Button>
                              </div>
                              ...
                              {question.options.map((option, optionIndex) => (
                                <div key={optionIndex} className="flex items-center gap-2">
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      const updatedModules = [...courseForm.modules];
                                      updatedModules[moduleIndex].quiz.questions[questionIndex].options[optionIndex] = e.target.value;
                                      setCourseForm({
                                        ...courseForm,
                                        modules: updatedModules
                                      });
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setCourseForm({
                        ...
                        modules: course.modules.map(({ id, ...module }) => module)
                      });
